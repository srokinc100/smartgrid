...
int tpe_security_bprm_check(struct linux_binprm *bprm)
{
    ...
    if(bprm->file)
    {
        ret = lac_allow_exec(bprm->file, "exec");
        if(IN_ERR(ret))
            goto out;
    }
        ...
        unsigned long prot)
{
    ...
    {
        ret = lac_allow_exec(vma->vm_file, "mprotect");
        if(IN_ERR(ret))
            goto out;
    }
...
void hijack_syscalls(void)
{
    ...
    {
        ret = symbol_hijack(security2hook[i].sym, 
                            security2hook[i].name, 
                            security2hook[i].func);
