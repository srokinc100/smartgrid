...
static inline pte_t *lac_lookup_address(unsigned long address, unsigned int *level)
{
...
#else
#define lac_lookup_address(address, level) lookup_address(address, level);
#endif
...
static inline void set_addr_rw(unsigned long addr, bool *flag)
{
    ...
    *flag = true;
    ...
    pte = lac_lookup_address(addr, &level);
